...
- **Tier-0 knobs** (scales/gains) are inline KO checkboxes; **expression knockouts**
  ...
  byte-identical.
- **Env gotcha:** the project is uv-managed overall, but `make tui` currently runs
  `pyenv exec python`, NOT bare `python3` — on dev machines `python3` may resolve
  to a system Python lacking `textual-plotext` / the sim deps. Deps:
