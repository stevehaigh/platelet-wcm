...
Raw Data (72+ TSV files)
  → ParCa (parameter fitting) → sim_data pickle
...
raw data → basal expression → TF conditions → condition fitting (optimization) → promoter binding (CVXPY) → final adjustments → `simData.cPickle`.
...
- **Imports**: absolute imports from repo root (`from wholecell.sim.simulation import Simulation`, `from models.ecoli.processes.metabolism import Metabolism`)
...
Two-layer build: `wcm-runtime` (Python + deps) → `wcm-code` (source + compiled extensions). Dockerfiles in `cloud/docker/`. Known build fixes for `Equation 1.2.1` and `stochastic-arrow 1.0.0` are applied in the runtime Dockerfile.
