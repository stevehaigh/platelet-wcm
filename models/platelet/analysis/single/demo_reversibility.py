...
class Plot(singleAnalysisPlot.SingleAnalysisPlot):
	...
	def do_plot(self, simOutDir, plotOutDir, plotOutFileName, simDataFile,
		...
		fig.subplots_adjust(wspace=0.26, top=0.85, bottom=0.18)

		# PAC-1 active fraction — the reversible activation
		ax = axes[0]
		draw(ax, 'IntegrinTrace', 'active_frac', 'tab:purple',
			r'PAC-1$^{+}$ active fraction')
		ax.set_ylabel(r'PAC-1$^{+}$ active fraction  (0–1)')
		ax.set_ylim(0, 1)
		...
		draw(ax, 'SecretionTrace', 'adp_e_uM', 'tab:cyan',
			r'secreted $[\mathrm{ADP}]_e$')
		ax.set_ylabel(r'secreted $[\mathrm{ADP}]_e$  ($\mu$M)')
		ax.set_ylim(bottom=0)
		ax.set_title('Autocrine $[\\mathrm{ADP}]_e$  (driver; cleared by '
			'ecto-NTPDase)', fontsize=10, fontweight='bold')
		ax.legend(loc='best', fontsize=8)
		...
		suptitle(fig, 'Integrin reversibility — PAC-1 relaxes as the ADP drive '
			'is withdrawn', base)
		provenance_footnote(fig, 'demo_reversibility', metadata)
