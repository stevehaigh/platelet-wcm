...
class Snapshot:
	...
	# ── PKC feedback hub (v0.6) ───────────────────────────────────────────
	pkc_active: int               # active PKC count (CalciumTrace)
	p2y1_desens_frac: float       # fraction of P2Y1 PKC-desensitised (0–1)
	plcb_phos_frac: float         # fraction of PLCβ PKC-phosphorylated (0–1)
	# ── Downstream PKC outputs (v0.61 / §3) ───────────────────────────────
	# Present only if the SecretionTrace / ThromboxaneTrace / IntegrinTrace
	# listeners exist in the run (older sims won't have them → has_downstream
	# is False and these stay 0, rendered as "—").
	has_downstream: bool
	adp_released_frac: float      # dense-granule ADP released (0–1)
	serotonin_released_frac: float
	fibrinogen_released_frac: float
	pselectin_surface_frac: float
	adp_e_uM: float               # autocrine pericellular ADP (µM)
	txa2_uM: float                # synthesised thromboxane A₂ (µM)
	tp_active_frac: float         # TP receptor active fraction (0–1)
	pac1_frac: float              # αIIbβ3 active fraction — PAC-1 readout (0–1)
...
def load_snapshots(simout: Path) -> tuple[list[Snapshot], Totals, dict]:
	...
	# PKC feedback hub (v0.6). Default to zeros for pre-v0.6 sims missing them.
	def _ca_col(name: str) -> np.ndarray:
		try:
			return ca_reader.readColumn(name).flatten()
		except (KeyError, ValueError):
			return np.zeros(len(t))
	pkc_active_col = _ca_col('pkc_active')
	p2y1_des_col = _ca_col('p2y1_desensitised_frac')
	plcb_phos_col = _ca_col('plcb_phosphorylated_frac')

	# Optional downstream listeners (v0.61 / §3) — absent in older runs.
	def _opt(listener: str, column: str) -> np.ndarray | None:
		d = simout / listener
		if not d.is_dir():
			return None
		try:
			return TableReader(str(d)).readColumn(column).flatten()
		except (KeyError, ValueError, OSError):
			return None

	sec_adp_col = _opt('SecretionTrace', 'adp_released_frac')
	sec_5ht_col = _opt('SecretionTrace', 'serotonin_released_frac')
	sec_fga_col = _opt('SecretionTrace', 'fibrinogen_released_frac')
	sec_psel_col = _opt('SecretionTrace', 'pselectin_surface_frac')
	sec_adpe_col = _opt('SecretionTrace', 'adp_e_uM')
	tx_uM_col = _opt('ThromboxaneTrace', 'txa2_uM')
	tx_tp_col = _opt('ThromboxaneTrace', 'tp_active_frac')
	itg_pac1_col = _opt('IntegrinTrace', 'active_frac')
	has_downstream = any(c is not None for c in
		(sec_adp_col, tx_uM_col, itg_pac1_col))

	def _at(arr: np.ndarray | None, i: int) -> float:
	...
	for i in range(len(t)):
		snaps.append(Snapshot(
			...
			pkc_active=int(pkc_active_col[i]),
			p2y1_desens_frac=float(p2y1_des_col[i]),
			plcb_phos_frac=float(plcb_phos_col[i]),
			has_downstream=has_downstream,
			adp_released_frac=_at(sec_adp_col, i),
			serotonin_released_frac=_at(sec_5ht_col, i),
			fibrinogen_released_frac=_at(sec_fga_col, i),
			pselectin_surface_frac=_at(sec_psel_col, i),
			adp_e_uM=_at(sec_adpe_col, i),
			txa2_uM=_at(tx_uM_col, i),
			tp_active_frac=_at(tx_tp_col, i),
			pac1_frac=_at(itg_pac1_col, i),
		))
...
def _visible_len(markup: str) -> int:
	"""Terminal cell width of a rich-markup string, ignoring the markup tags.

	Replaces the hand-counted "visible length" literals the schematic used
	to carry alongside every line — `Text.from_markup(...).cell_len` is the
	single source of truth for how wide a line renders, so layout padding
	can't drift out of sync with the markup."""
	return Text.from_markup(markup).cell_len


def _pct(frac: float) -> str:
	"""Clamp a 0–1 fraction to a 0–100 integer-percent string."""
	return f'{max(0, min(100, int(round(frac * 100)))):>3d}%'
...
def _cell_schematic(s: Snapshot, totals: Totals, ca_ex_uM: float,
	...
	# Cytosol header
	inside_lines.append(inside('  [b cyan]Cytosol[/b cyan]'))
	...
	# PKC feedback hub (v0.6): active PKC + the two brakes it drives
	# (P2Y1 desensitisation, PLCβ phosphorylation).
	inside_lines.append(inside(
		f'  [b]PKC[/b]  active {s.pkc_active:>5}   '
		f'P2Y1-desens [yellow]{_pct(s.p2y1_desens_frac)}[/yellow]   '
		f'PLCβ-phos [yellow]{_pct(s.plcb_phos_frac)}[/yellow]'))
	...
	if s.has_downstream:
		...
		inside_lines.append(inside(
			...
			f'  α-FGA {_pct(s.fibrinogen_released_frac)}'
			f'  P-sel {_pct(s.pselectin_surface_frac)}'
			f'   → ADP\\[e] {s.adp_e_uM:>5.2f} µM'))   # \\[ = literal [ in rich markup
		inside_lines.append(inside(
			f'  [b]TXA₂[/b] {s.txa2_uM:>5.2f} µM  TP {_pct(s.tp_active_frac)}'
			f'      [b]Integrin[/b] αIIbβ3  '
			f'[b green]PAC-1 {_pct(s.pac1_frac)}[/b green]'))
